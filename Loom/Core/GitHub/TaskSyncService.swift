...
actor TaskSyncService {
    ...
    init(database: LoomDatabase, rest: RESTClient, graphql: GraphQLClient) {
    ...
    func fullSync() async throws {
        ...
        guard !trackedRepos.isEmpty else {
            LoomLog.sync.info("No tracked repos; sync is a no-op")
            return
        }
        ...
        LoomLog.sync.info("Starting full sync over \(trackedRepos.count, privacy: .public) tracked repos")
        ...
        LoomLog.sync.info("Full sync complete")
    }
...
enum TaskSyncWrites {
    ...
    static func deleteStaleTasks(db: Database, repos: [Repo], fetched: [RawTask]) throws {
        ...
        for repo in repos {
            ...
            let allForRepo = try LoomTask.fetchAll(
