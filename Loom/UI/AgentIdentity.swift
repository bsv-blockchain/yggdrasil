...
struct AgentBadge: View {
        ...
        ZStack(alignment: .bottomTrailing) {
            ...
            if let icon = statusIcon, showsPip(for: icon) {
                Circle()
                    ...
                    .overlay(
                        Circle()
                            .stroke(LoomTheme.bgPane(scheme), lineWidth: 1.5)
                    )
    ...
    private func color(for icon: TabStatus.Icon) -> Color {
        switch icon {
        case .errored, .ciFailing: LoomTheme.statusErr(scheme)
        case .awaitingInput, .dirty: LoomTheme.statusWarn(scheme)
        case .running: LoomTheme.statusInfo
        case .unread: LoomTheme.ember
        case .idle: LoomTheme.statusIdle(scheme)
        }
    }
}
