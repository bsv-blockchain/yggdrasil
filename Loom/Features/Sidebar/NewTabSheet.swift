...
/// "+" sheet shown from the sidebar — redesigned per `Loom.html`'s agent
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if let error {
                Text(error)
                    .font(.callout)
                    .foregroundStyle(LoomTheme.statusErr(scheme))
        ...
        HStack(spacing: 10) {
            ...
            if let repo = repos.first(where: { $0.id == selectedRepoID }),
               let path = repo.localMainPath {
                ...
                Text("(no local clone)")
                    .font(.system(size: 10))
                    .foregroundStyle(LoomTheme.statusWarn(scheme))
            }
        }
    ...
    var body: some View {
            ...
            HStack(spacing: 8) {
                ...
                if agent.isDefault {
                    Text("DEFAULT")
                        ...
                        .background(
                            Capsule().fill(LoomTheme.accentSoft(scheme))
                        )
                }
            }
        ...
        .background(
            RoundedRectangle(cornerRadius: 10)
                .fill(LoomTheme.bgPaneSoft(scheme))
        )
        .overlay(
            RoundedRectangle(cornerRadius: 10)
                .stroke(
                    isSelected
                        ? LoomTheme.accent
                        : LoomTheme.border(scheme),
                    lineWidth: isSelected ? 1.2 : 0.5
                )
        )
        .shadow(
            color: isSelected ? LoomTheme.accent.opacity(0.15) : .clear,
            radius: 6, x: 0, y: 2
        )
    }
}
