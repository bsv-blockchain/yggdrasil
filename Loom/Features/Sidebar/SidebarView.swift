...
/// Sidebar — redesigned per `Loom.html`:
/// - Header: blue gradient Loom mark + workspace name + "N tabs · M active"
...
struct SidebarView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            filterPills
            Divider()
                .background(LoomTheme.divider(scheme))
            ...
            Text("⌘K")
                ...
                .background(
                    RoundedRectangle(cornerRadius: 3)
                        .fill(LoomTheme.chipBg(scheme))
                        .overlay(
                            RoundedRectangle(cornerRadius: 3)
                                .stroke(LoomTheme.chipBd(scheme), lineWidth: 0.5)
                        )
                )
        }
            ...
            ForEach(Filter.allCases) { filter in
                Button {
                    ...
                    Text(filter.rawValue)
                        ...
                        .foregroundStyle(
                            activeFilter == filter ? LoomTheme.text(scheme) : LoomTheme.textMute(scheme)
                        )
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            Image(systemName: "tray")
                .font(.system(size: 32))
                .foregroundStyle(LoomTheme.textFaint(scheme))
            Text("No tabs yet")
                .font(.headline)
                .foregroundStyle(LoomTheme.text(scheme))
...
/// Loom mark — interwoven threads. Mirrors `icons.jsx` "loom" path on a 16×16
    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: 6)
                .fill(
                    LinearGradient(
                        colors: [LoomTheme.accent, LoomTheme.accentDeep],
