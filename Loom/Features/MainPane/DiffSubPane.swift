...
/// into the page via `window.loom.render(diffText)`.
///
...
struct DiffSubPane: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSView {
        ...
        if let url, FileManager.default.fileExists(atPath: url.path) {
            ...
            LoomLog.ui.error("Bundled diff2html/index.html not found in Loom.app/Resources")
        }
        return host
    }
    ...
    final class Coordinator: NSObject, WKNavigationDelegate {
        ...
        init(services: AppServices, tab: LoomTab) {
        ...
        private func computeAndPush(webView: WKWebView) async {
            do {
                ...
                let escaped = payload.replacingOccurrences(of: "\\", with: "\\\\")
                    ...
                    .replacingOccurrences(of: "$", with: "\\$")
                let script = "window.loom && window.loom.render(`\(escaped)`);"
                ...
                let escapedErr = String(describing: error)
                    .replacingOccurrences(of: "`", with: "'")
                let script = "window.loom && window.loom.render(`# Diff failed\\n\\n\(escapedErr)`);"
                _ = try? await webView.evaluateJavaScript(script)
            }
        }
        ...
        private func truncatedPlaceholder(diff: UnifiedDiff) -> String {
            """
            diff --git a/LOOM-NOTICE b/LOOM-NOTICE
