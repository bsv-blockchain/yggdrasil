...
/// Right-hand pane of the main window. Per `Loom.html` the layout is a
...
struct MainPaneView: View {
    ...
    private func pane(_ segment: PaneSegment) -> some View {
        VStack(spacing: 0) {
            ...
            Rectangle()
                .fill(LoomTheme.border(scheme))
                .frame(height: 0.5)
            content(for: segment)
        }
        .background(LoomTheme.bgPane(scheme))
...
/// One renderable surface in a pane. Distinct from `LoomTab.MainView` because
...
enum PaneSegment: Hashable {
    ...
    var tabMainView: LoomTab.MainView {
        switch self {
        case .agent: .agent
        case .github: .github
        case .diff: .diff
    ...
    static func from(_ view: LoomTab.MainView) -> PaneSegment {
        switch view {
        case .agent: .agent
        case .github: .github
        case .diff: .diff
...
enum PaneLayout: Hashable {
    ...
    /// Default per the design's `LOOM_DEFAULTS` ("terminal+diff"), with the
    /// per-tab `last_main_view` as a fallback when no global override is set.
    static func load(for tab: LoomTab) -> PaneLayout {
        if let raw = UserDefaults.standard.string(forKey: "loom.paneLayout"),
    ...
    func persist(for tab: LoomTab) {
        UserDefaults.standard.set(rawValue, forKey: "loom.paneLayout")
    }
...
struct PaneHeader: View {
    ...
    var body: some View {
        ...
        .frame(height: 38)
        .background(LoomTheme.bgWindow(scheme))
    }
    ...
    private var segmentedControl: some View {
        ...
        .padding(2)
        .background(LoomTheme.bgPaneSoft(scheme))
        .overlay(
            RoundedRectangle(cornerRadius: 7)
                .stroke(LoomTheme.border(scheme), lineWidth: 0.5)
        )
    ...
    private func segmentButton(_ target: PaneSegment, label: String) -> some View {
        ...
        Button {
            ...
            .foregroundStyle(
                isSelected
                    ? (target == .agent ? agentIdentity.color : LoomTheme.text(scheme))
                    : LoomTheme.textMute(scheme)
            )
            .background(
                RoundedRectangle(cornerRadius: 5)
                    .fill(isSelected ? LoomTheme.bgElev(scheme) : Color.clear)
            )
        }
        .buttonStyle(.plain)
    }
