...
Date: 2026-05-26
Spec reference: `yggdrasil-spec.md` §Phase 0 (lines 226–251)
...
- Created the full `Yggdrasil/`, `Core/<sub>`, `Features/<sub>`, `UI/`, `Resources/`, and `Tests/<sub>` tree per the spec. Empty directories carry a `.gitkeep` so the structure survives commit.

### Project authoring
- `project.yml` (XcodeGen spec) is the source of truth.
- `make project` regenerates `Yggdrasil.xcodeproj`. The generated project is also committed so `xcodebuild` works without XcodeGen installed.
- Single app target `Yggdrasil` (macOS 14, Swift 5.10, hardened runtime, ad-hoc signing for local dev).
- `Yggdrasil.entitlements`: network client + user-selected file access; **no App Sandbox** (rationale in `decisions.md`).

### Code
- `Yggdrasil/App/YggdrasilApp.swift` — `@main` SwiftUI App vending a single `WindowGroup` titled "Yggdrasil" containing a `RootView` placeholder.
- `Yggdrasil/App/AppDelegate.swift` — `NSApplicationDelegate` with launch/terminate logging; quits after last window closes.
- `Yggdrasil/Core/Logging/YggdrasilLogger.swift` — `YggdrasilLog` namespace exposing `os.Logger` for the six subsystem categories required by the spec: `sync`, `pty`, `git`, `ui`, `db`, `auth`. Subsystem string `com.bsvassociation.yggdrasil` matches the app bundle identifier.
- `Yggdrasil/Core/DependencySmokeImports.swift` — imports all four dependencies and calls a libgit2 symbol to prove dynamic linkage (not just import-resolution).
...
### Tests
- `Tests/Unit/SmokeTests.swift` — 5 tests in the `YggdrasilTests` bundle:
  1. `testAppVendsAWindowTitledYggdrasil` — verifies a window titled "Yggdrasil" exists (runs in-process, host = `Yggdrasil.app`).
...
### State files
- `.yggdrasil-build-state.json` — `current_phase: 0`, started_at set, last_git_sha to be set at commit.
...
Spec mandated a committed `Yggdrasil.xcodeproj` but didn't say how to maintain it. I chose XcodeGen with both `project.yml` and the generated `Yggdrasil.xcodeproj` committed. The pbxproj is reproducible. Logged in `decisions.md`.
...
Test Suite 'YggdrasilTests.xctest' started at 2026-05-26 19:17:58.807
Test Suite 'SmokeTests' started at 2026-05-26 19:17:58.807
Test Case '-[YggdrasilTests.SmokeTests testAppVendsAWindowTitledYggdrasil]' passed (0.001s)
Test Case '-[YggdrasilTests.SmokeTests testBundleIdentifierMatchesSpec]' passed (0.000s)
Test Case '-[YggdrasilTests.SmokeTests testDependencySmokeImportsAllFourPackages]' passed (0.001s)
Test Case '-[YggdrasilTests.SmokeTests testLibgit2LinksAndReportsAVersion]' passed (0.001s)
Test Case '-[YggdrasilTests.SmokeTests testLoggerSubsystemHasExpectedCategories]' passed (0.000s)
...
Also confirmed via `Yggdrasil did finish launching (pid=…)` line in the log — proves AppDelegate fires and the SwiftUI scene comes up under TEST_HOST.
...
1. **Push to a GitHub remote** — `bsv-blockchain/yggdrasil`? Personal account? Yggdrasil is your own project so I default to recommending a personal account remote first; we can transfer later. CI's green-light AC #4 unblocks the moment the workflow runs there.
2. **Bundle ID** — locked to `com.bsvassociation.yggdrasil`. If you'd prefer a different reverse-DNS prefix, easiest to swap before signing arrives in Phase 8.
