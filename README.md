...
time of writing).** Built phase-by-phase against [`yggdrasil-spec.md`](yggdrasil-spec.md);
...
- The `gh` CLI authenticated against your GitHub account (Yggdrasil uses it as the
...
| A tab's terminal stays empty | Check Console.app filtered to subsystem `com.bsvassociation.yggdrasil` — the agent's stderr is in there. |
| "Repo has no local clone path" when opening a tab | Preferences → Repos → Choose…  Yggdrasil can't create a worktree without the local clone. |
| Yggdrasil appears to hang | `Help → Diagnostics` copies a quick report to your clipboard; paste it into a bug report. |
| Stale GitHub login | The persistent web data store lives at `~/Library/WebKit/com.bsvassociation.yggdrasil/WebsiteData/`. Delete it to log out. |

For deeper inspection:

```bash
log show --predicate 'subsystem == "com.bsvassociation.yggdrasil"' --last 5m
...
See [`yggdrasil-spec.md`](yggdrasil-spec.md) §3.3. Source under `Yggdrasil/`, tests under
`Tests/`, libgit2 system-library wrapper at `Vendor/Clibgit2/`, bundled
diff2html at `Yggdrasil/Resources/diff2html/`.

## Phase progress

- `.yggdrasil-build-state.json` — machine-readable build state (current phase, SHAs).
