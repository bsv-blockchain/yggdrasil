# Yggdrasil — Build Spec for Claude Code
...
5. Update `.yggdrasil-build-state.json`: bump `current_phase`, append to `completed_phases`, record git SHA.
...
- **`.yggdrasil-build-state.json`** — machine-readable progress. Schema:
  ```json
  {
    "current_phase": 0,
    "completed_phases": [],
...
2. Initialise `.yggdrasil-build-state.json` with `current_phase: 0`.
...
| Local storage | GRDB.swift (SQLite) | `~/Library/Application Support/Yggdrasil/yggdrasil.sqlite` |
...
```mermaid
flowchart LR
    subgraph App["Yggdrasil (single macOS process)"]
        UI["SwiftUI views<br/>Sidebar · Main pane (Agent/GitHub/Diff)"]
        VM["ViewModels<br/>@Observable, actor-isolated"]
        Store[("GRDB SQLite<br/>~/Library/Application Support/Yggdrasil")]
...
Makefile
README.md
yggdrasil-spec.md           # this file lives here too
.yggdrasil-build-state.json
coverage-ledger.md
decisions.md
...
**Deliverables:**
- Xcode project committed (`Yggdrasil.xcodeproj`), single app target, signing configured for local dev.
...
- `Yggdrasil.entitlements` with file access, network access. No sandbox initially (document why in `decisions.md`).
- `.yggdrasil-build-state.json`, `coverage-ledger.md`, `decisions.md` initialised.

**Acceptance criteria:**
- [ ] `make build` succeeds from clean checkout
- [ ] `make test` succeeds, runs ≥ 1 test
- [ ] App launches, shows an empty NSWindow titled "Yggdrasil", quits cleanly via ⌘Q
- [ ] CI green on GitHub
...
- File lock (POSIX advisory lock on `<repo>/.worktrees/.yggdrasil.lock`) to serialise concurrent worktree ops on the same repo.
...
- Crash reporter — local file in `~/Library/Logs/Yggdrasil/crashes/` with a "Reveal in Finder" action in the menu.
...
- **Tab** — a row in Yggdrasil's sidebar. May or may not be linked to a GitHub task. Has a worktree, and optionally a running coding-agent session.
- **Task** — a GitHub issue or PR assigned to the user.
- **Tracked repo** — a repo the user has added; Yggdrasil syncs assigned tasks from it.
...
- **Session** — a single PTY-hosted process running one **coding agent** in a worktree. Yggdrasil does not run generic shells; users wanting a shell open Terminal.app.
...
- Multi-window mode
- Plugins / extensions
- Issue triage UI (create/edit issues inside Yggdrasil)
- Branch / PR creation from inside Yggdrasil (open the GitHub WebView to do it)
