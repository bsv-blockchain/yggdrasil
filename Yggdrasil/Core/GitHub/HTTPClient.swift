...
/// HTTP response from a Yggdrasil REST/GraphQL call after the client applied auth,
...
final class URLSessionHTTPClient: HTTPClient {
    ...
    init(session: URLSession, auth: AuthService, etags: ETagStore, userAgent: String = "Yggdrasil/0.1") {
    ...
    private func perform(
        ...
        do {
            ...
            YggdrasilLog.sync.error("HTTP request failed: \(error.code.rawValue) \(error.localizedDescription)")
        ...
        if let remaining, remaining < 100 {
            YggdrasilLog.sync.warning("GitHub rate-limit remaining: \(remaining, privacy: .public)")
        }

        if http.statusCode == 401 {
            if attempt == 1 {
                YggdrasilLog.auth.info("Got 401 — invalidating cached token and retrying once")
