...
Spec reference: `loom-spec.md` §Phase 1 (lines 254–281)
...
- `LoomDatabase` — GRDB stack. `openDefault()` for production (`~/Library/Application Support/Loom/loom.sqlite`), `inMemory()` for tests.
- `Migrations` v1: full §3.2 schema — `repo`, `task`, `task_assignee`, `github_status`, `setting`.
- 5 Codable+Record models: `Repo`, `LoomTask` (renamed from `Task` to avoid collision with `_Concurrency.Task` — table stays `task`), `TaskAssignee`, `GitHubStatus`, `Setting`.
...
- `KeychainStore` protocol + `KeychainAccessStore` (real, service = `com.bsvassociation.loom`, accessibility = `.afterFirstUnlock`).
...
### HTTP (`Loom/Core/GitHub/{Backoff,BackoffRetry,GitHubError,HTTPClient}.swift`)
...
- `URLSessionHTTPClient`:
  ...
  - Parses `X-RateLimit-Remaining`, warns to `LoomLog.sync` below the 100-call threshold per spec.
...
### API clients (`Loom/Core/GitHub/{Endpoints,APIResponses,RESTClient,GraphQLClient}.swift`)
...
### Sync orchestration (`Loom/Core/GitHub/TaskSyncService.swift`, `SyncScheduler.swift`)
...
### App wiring (`Loom/App/{AppDelegate,AppServices,DebugMenu,LoomApp}.swift`)
...
- Now sets `env: LOOM_TEST_GITHUB_TOKEN: ${{ secrets.LOOM_TEST_GITHUB_TOKEN }}` on the `make test` step so the integration test runs when the user adds the secret.
...
### 3.3 `LoomTask` instead of `Task`
The DB type uses `LoomTask` to avoid colliding with Swift Concurrency's `_Concurrency.Task`. Database table name stays `task` per spec §3.2. Logged in commit message.
...
1. `make build` then run `open ~/Library/Developer/Xcode/DerivedData/Loom-*/Build/Products/Debug/Loom.app`. The window titled "Loom" should appear.
2. Open Console.app → filter by subsystem `com.bsvassociation.loom`. You'll see `[ui] Loom did finish launching (pid=…)` and `[sync] No tracked repos; sync is a no-op`.
...
6. Inspect SQLite: `sqlite3 ~/Library/Application\ Support/Loom/loom.sqlite "SELECT * FROM task;"`

The 60s background sync ticks should also appear in Console (`[sync] Starting full sync…` lines at ~60s intervals).

To run the live integration test locally: `LOOM_TEST_GITHUB_TOKEN=$(gh auth token) make test`.

---

## 5. Open questions

1. **GitHub remote** — still no remote configured. AC #4 from Phase 0 (CI green on GitHub) and the integration test secret remain `[BLOCKED]` until you push the repo somewhere. When you do: add `LOOM_TEST_GITHUB_TOKEN` as a repo secret. Want me to draft the `git remote add` + first push when you give the URL?
